    ...
    <div className="min-h-screen bg-gray-100 text-gray-800 flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* 로그아웃 */}
          <button onClick={() => window.location.href = '/'} className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors" title={lang === 'ko' ? '로그아웃' : 'Logout'}>
            ...
            <div className="flex items-end gap-2 flex-wrap">
              {currentInterface.params.map((param) => {
                ...
                return (
                  <div key={param} className="flex flex-col gap-0.5">
                    <label className="text-[10px] text-gray-500">{getParamLabel(param, lang)}{isParamRequired(currentInterface.id, param) && <span className="text-red-500 ml-0.5">*</span>}</label>
                    <input type="text" value={paramValues[param] || ''} onChange={(e) => setParamValues({ ...paramValues, [param]: e.target.value })} placeholder={param === 'I_ZPLDAYS' ? '일수' : ''}
